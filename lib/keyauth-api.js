...
async function handleRegister(req, res) {
  ...
  return success(res, 'Logged in!', {
    info: {
      ...
      subscriptions: [{ subscription: user.subscription?.toISOString(), key: license.key }],
      ip: user.ip,
      hwid: user.hwid,
...
async function handleLogin(req, res) {
  ...
  return success(res, 'Logged in!', {
    info: {
      username: user.username,
      subscriptions: [{ subscription: user.subscription?.toISOString(), key: '' }],
      ip: getClientIp(req),
      hwid: hwid || user.hwid,
      createdate: user.createdAt.toISOString(),
      lastlogin: new Date().toISOString(),
...
async function handleLicense(req, res) {
  ...
  if (!license) return failure(res, 'Invalid license key');
  if (license.used) return failure(res, 'License already used');
  ...
  return success(res, 'Logged in!', {
    info: {
      username: user.username,
      subscriptions: [{ subscription: subscription.toISOString(), key: license.key }],
      ip: getClientIp(req),
      hwid: hwid || user.hwid,
      createdate: user.createdAt.toISOString(),
      lastlogin: new Date().toISOString(),
  ...
  return success(res, 'Session valid', {
    info: session.user
      ? {
          ...
          subscriptions: [{ subscription: session.user.subscription?.toISOString() }],
        }
      : {},
  });
}
